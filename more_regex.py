...
time_string = "3:45 PM"
pat = re.compile(r"(?P<hour>\d{1,2})+:+(?P<minute>\d{2})+\s+(?P<AMPM>[A-P]M)", re.IGNORECASE)
...
pat1 = re.compile(r"\w{5,20}", re.IGNORECASE)
print(pat1.findall(text_string))
pat2 = re.compile(r"\w*[AEIOUaeiou]\w*")
FiveOrMore = " ".join(pat1.findall(text_string))
word_matches = pat2.findall(FiveOrMore)
...
pat = re.compile(r"(\w+):\/\/(\w+\.\w+)([\/\w+]*)", re.IGNORECASE)
...
pat = re.compile(r"\d{3}-\d{3}-\d{4}|\(\d{3}\)\s+\d{3}-\d{4}|\d{10}", re.IGNORECASE)
